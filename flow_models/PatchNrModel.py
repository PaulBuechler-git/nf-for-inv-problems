...
def multi_layer_nr_net(layers, hidden_layer_sub_net, input_dim_flat):
    ...
    split = ff.Node(graph[-1], fm.Split1D, {'dim':0}, name=f'Split')
    graph.append(split)
    graph.append(ff.Node(graph[-1],
                         fm.GLOWCouplingBlock,
                         {'subnet_constructor': net_fn, 'clamp': 1.6}, name=f'glow block'))
    graph.append(ff.Node(graph[-1], fm.PermuteRandom, {}, name=f'permute random'))
    graph.append(ff.Node([split.out1, graph[-1].out0], fm.Concat, {}, name='concat'))
...
class PatchNrModel(pl.LightningModule):
    ...
    def call_model(self, patch, rev: bool = True):
        return self.patch_nr_model(patch, rev=True)
    ...
    def validation_step(self, batch, batch_id):
        ...
        z, z_jac = self.patch_nr_model(batch, rev=True)
        loss += torch.mean(0.5 * torch.sum(z ** 2, dim=1) - z_jac)
        self.mean_validation_loss.append(loss)
        return loss

    def on_validation_epoch_end(self):
        ml = torch.mean(torch.stack(self.mean_validation_loss))
        self.log_dict({"mean_loss": ml})
        self.mean_validation_loss = []
