...
if __name__ == '__main__':
    ...
    patchNR = PatchNrModel.get_model(num_layers, subnet_nodes, patch_size**2)
    ...
    for k in tqdm(range(optimizer_steps)):
        ...
        #compute loss
        loss = 0
        invs, jac_inv = patchNR.inverse_and_log_det(patch_example)
        ...
        if k%1000 ==0: 
            with torch.no_grad():
                ...
                invs, jac_inv = patchNR.inverse_and_log_det(val_patch, rev = True)
        ...
        if (k+1) % 50000 == 0:
            it = int((k+1)/1000)
            torch.save({'net_state_dict': patchNR.state_dict()}, 'patchNR_custom/weights_'+image_class + '_'+str(it) + '.pth')
    torch.save({'net_state_dict': patchNR.state_dict()}, 'patchNR_custom/weights_'+image_class + '.pth')
